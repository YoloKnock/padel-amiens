 ...
 * "P 100 DAME" → P100, "P50 JOURNÉE" → P50, etc.
 */
 ...
 * Heuristique : DAMES > MIXTE > MESSIEURS (par défaut messieurs si rien trouvé).
 */
export function parseGender(title: string): Gender {
  ...
  if (upper.includes('MIXTE')) {
    return 'mixte';
  }
  // "Messieurs", "Hommes", ou rien spécifié → messieurs par défaut
  return 'messieurs';
}
...
export function parseHtmlPage(html: string): {
  ...
  $('h5, h4, h3, [class*="date"]').each((_, el) => {
    ...
    const cityMatch = blockText.match(/\d{5}\s+([A-ZÉÈÀÇÔÎ][A-ZÉÈÀÇÔÎa-zéèàçôî\s-]+)/);
